...
from love.modules.helper_funcs.extraction import extract_user_and_text
from love.modules.helper_funcs.string_handling import extract_time
from love.modules.log_channel import gloggable, loggable
...
def kick(update: Update, context: CallbackContext) -> str:
    ...
    if res:
        ...
        bot.sendMessage(
            chat.id,
            f"Fuck 🙂Off! {mention_html(member.user.id, html.escape(member.user.first_name))}.",
            parse_mode=ParseMode.HTML,
        )
        log = (
            f"<b>{html.escape(chat.title)}:</b>\n"
...
def kickme(update: Update, context: CallbackContext):
    ...
    if res:
        update.effective_message.reply_text("Go out of 😒here")
    else:
