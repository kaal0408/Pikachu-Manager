...
# This file is part of Kristina (Telegram Bot)
...
def add_chat(update: Update, context: CallbackContext):
    ...
    if not is_kuki:
        ...
        msg.reply_text("Lovish AICHAT successfully enabled for this chat!")
        message = (
        ...
        return message
    msg.reply_text("Lovish AICHAT is already enabled for this chat!")
    return ""
...
def rem_chat(update: Update, context: CallbackContext):
    ...
    is_kuki = sql.is_kuki(chat.id)
    if not is_kuki:
        msg.reply_text("Lovish AICHAT isn't enabled here in the first place!")
        return ""
    sql.rem_kuki(chat.id)
    msg.reply_text("Lovish AICHAT disabled successfully!")
    message = (
        f"<b>{html.escape(chat.title)}:</b>\n"
        f"AI_DISABLED\n"
        f"<b>Admin:</b> {mention_html(user.id, html.escape(user.first_name))}\n"
...
__help__ = """
Chatbot utilizes the Branshop's API and allows Lovish to talk and provides a more interactive group chat experience.
*Commands:*
*Admins only:*
